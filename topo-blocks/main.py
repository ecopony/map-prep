...
class TopoBlockDesigner:
    ...
    def create_block_design(self, mountain_name: str, output_path: Union[str, Path], 
                          ...
                          gap_percent: float = 0.01, 
                          dpi: int = 300,
                          figsize: Tuple[float, float] = (12, 12),
                          line_width: float = 0.8,
                          background_color: str = 'transparent',
                          text_size: Optional[int] = None,
                          adaptive_text: bool = True) -> None:
        """
    ...
    def batch_create_designs(self, mountain_name: str, output_folder: Union[str, Path], 
        ...
        if color_schemes is None:
            color_schemes = {
                'sunset': ['#FF6B6B', '#FF8E8E', '#FFB3B3'],
                'ocean': ['#1A237E', '#3F51B5', '#5C6BC0'],
                'forest': ['#2E7D32', '#4CAF50', '#81C784'],
                'earth': ['#8D6E63', '#A1887F', '#BCAAA4'],
                'neon': ['#FF1744', '#00E676', '#00B0FF'],
                'monochrome': ['#2C2C2C', '#5C5C5C', '#8C8C8C'],
                'warm': ['#D84315', '#FF5722', '#FF8A65'],
                'cool': ['#0277BD', '#03A9F4', '#4FC3F7'],
                'mine': ['#9AD2CB', '#D7EBBA', '#FEFFBE']
            }
...
def main():
    ...
    try:
        if args.batch and args.batch_output:
            ...
            results = batch_process_mountains(args.batch, args.batch_output)
