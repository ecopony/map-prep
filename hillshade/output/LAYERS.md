...
**Script**: `generate_hillshade.py` (commit `46224f7`, 2024-06-05)
...
**Relationship to `mapprep/hillshade.py`**: the shared module did *not* originate from this project — git shows it descends from `utils/hillshade_utils.py` (commit `b709885`, 2024-01-26, predating this project) via the uv-workspace conversion (`be271b7`). No code is shared between the two. The shared module's `from_dem` (GDAL DEMProcessing with `multiDirectional=True`) now covers the plain-hillshade use case that this script implements by hand (`combine_hillshades` averaging four azimuths), so for new work prefer `mapprep.hillshade.from_dem`; this script's value is the layered tint/mist/slope compositing, which has no shared-module equivalent.
...
- **Processing**: GDAL `DEMProcessing` hillshade with `computeEdges=True`, `zFactor=1` (`generate_hillshade`), via `combine_hillshades([315], [45])` (a single direction, so the averaging is a no-op); grayscale array converted to RGB PNG.
...
- **Source**: `input/oblique-clip.tif`.
- **Processing**: `combine_hillshades` with azimuths [45, 135, 225, 315], all at altitude 45, equal weights — four GDAL hillshades averaged with `np.average` and clipped to 0–255.
...
- **Source**: `input/oblique-clip.tif`.
- **Processing**: `combine_hillshades([315], [25])`, same GDAL hillshade path as stage 4.
...
- **Source**: `input/oblique-clip.tif`.
- **Processing**: GDAL `DEMProcessing` slope (`scale=1`, in-memory), nodata masked, min-max normalized (`generate_slope_array`), colored with matplotlib `cividis`, converted to RGB.
...
- **Role**: Byproducts, not deliverables — per-direction GDAL hillshade rasters (`temp_hillshade_315_45`, `_45_45`, `_135_45`, `_225_45`, `_315_25`) written by `generate_hillshade` because GDAL `DEMProcessing` needs an output path; read back immediately and never cleaned up.
- **Source**: `input/oblique-clip.tif`.
- **Processing**: GDAL hillshade, `computeEdges=True`, `zFactor=1`, one file per (azimuth, altitude) pair.
- **Rationale** *(reconstructed)*: Implementation convenience only (an in-memory `MEM` dataset, as used for slope, would avoid them).
