...
- The root `pyproject.toml` declares workspace members. Shared code lives in the `mapprep/` package (`mapprep/src/mapprep/`); converted projects (`ambient-occlusion`, `carson-rem`, `conferences`, `cropland`, `forest-loss`, `hillshade`, `property-rainwater`, `rainfall`) each have a `pyproject.toml` declaring their dependencies plus `mapprep = { workspace = true }`.
