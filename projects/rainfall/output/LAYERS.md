...
- **Processing**: `mapprep/natural_earth.py` `us_populated_places(min_population=500_000, contiguous=True, exclude=...)` — US places with `pop_max >= 500,000`, excluding Alaska, Hawaii, Puerto Rico, minus a hand-picked clutter list (`excluded_places` in `main.py`): St. Paul, Ft. Worth, San Bernardino, San Jose, Bridgeport, Vancouver WA, Baltimore, Rochester, Syracuse.
- **Rationale** *(reconstructed)*: 500k threshold keeps the city count manageable at CONUS scale; `pop_max` is metro-area population, so this reads as "major metros". *(2026-07-18)* Dropped 9 clutter labels rather than raising the threshold (raising it to ~800k was considered and rejected because it also removed regionally important solo labels like New Orleans and Oklahoma City). The dropped cities are satellites whose metro is already labeled by a bigger neighbor (St. Paul/Minneapolis, Ft. Worth/Dallas, San Bernardino/LA, San Jose/San Francisco, Bridgeport/NYC, Vancouver/Portland, Baltimore/Washington) or an upstate-NY pile-up thinned to Buffalo (Rochester, Syracuse). Baltimore is the judgment call to restore first if it feels missed.
- **Facts** *(extracted 2026-07-18)*: EPSG:4326, Point, 48 features, fields [scalerank, natscale, labelrank, featurecla, name, namepar, namealt, nameascii, … (31 total)]
