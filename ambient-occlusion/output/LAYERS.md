...
**Script**: `main.py` (commit `6737d08`, 2024-10-05, "Update ambient occlusion creation"; refactored 2026-08-06 onto the shared `mapprep` helpers — filenames below reflect the refactored script, which drops the old `_315.0_45.0` azimuth/altitude suffixes)
...
- **Processing**: `mapprep/raster.py` `blur` — scipy `uniform_filter` (mean filter) with window sizes 10, 20, and 50 pixels; the DEM is reflect-padded by half the window first so edges don't darken. Profile copied from the source.
...
## <area>/original_hillshade.tif, blur_10_hillshade.tif, blur_20_hillshade.tif, blur_50_hillshade.tif
...
- **Processing**: `mapprep/hillshade.py` `from_dem` — GDAL `DEMProcessing` hillshade with defaults azimuth 315, altitude 45, zFactor 1, `computeEdges=True`.
...
- **Source**: `input/combined_clip.tif` and the `blur_*.tif` intermediates above.
- **Processing**: `mapprep/dem.py` `slope` — GDAL `DEMProcessing` slope, zFactor 1, `computeEdges=True`.
...
- **Processing**: `mapprep/hillshade.py` `from_dem` applied to the slope raster, same defaults (315/45, zFactor 1, `computeEdges=True`).
