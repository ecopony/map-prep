...
- **Processing**: `mapprep/natural_earth.py` `us_states()` (since the 2026-08-06 refactor; previously an inline geopandas read with the same `iso_a2 == 'US'` filter) — no contiguous-US filter, so Alaska, Hawaii, and territories flagged `US` are included.
